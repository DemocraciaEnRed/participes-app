  ...
  @if(count($objectives) > 0)
    ...
    <div class="card mb-2">
      <div class="card-body">
        ...
        <h6 class="card-subtitle mb-2"><span class="badge badge-primary badge-pill">{{count($objective->goals)}}</span> Metas <small class="text-muted">Creado {{$objective->created_at->diffForHumans()}}</small></h6>
        <p class="text-muted text-smaller mb-0">{{$objective->content}}</p>
      </div>
    </div>
    @endforeach
  @else
